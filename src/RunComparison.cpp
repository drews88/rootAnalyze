...
void multiPlot(std::vector<std::string> dirPaths, std::string filePath, std::vector<std::string> runTitles, std::string title, std::string type, bool save = false, std::string savePath = "./newPlot.pdf") {
    ...
    */
    ...
    // Define a larger set of colors for more histograms
    ...
    for (const auto &dirPath : dirPaths) {
        ...
        hist->SetFillColorAlpha(colors[colorIndex % colorCount], 0.5); // Semi-transparent fill
        hist->SetLineWidth(2);
...
void RunComparison() {
    ...
    */
    ...
    std::string savePath = "../plots/runComp/Spectra/Dark/newCleanedComparison.pdf";
